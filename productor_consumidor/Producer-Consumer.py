...
colum = 2 #Producidos
pos = 2 #Consumidos
consumidor = random.randint(3,10) #Consumidor
productor = random.randint(3,10) #Productor
...
while not exit:
    ...
    if band == 0: #Turno del productor
        ...
        while (producidos < productor): #Elementos a producir 
            if exit:
                break
            elif  len(cont) == 35: #Si el contenedor (buffer) esta lleno, no se puede producir 
                ...
                imprimir_en_posicion(1,1,colum)
                cont.append('$')
